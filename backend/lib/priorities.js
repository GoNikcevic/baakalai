 ...
 * ramenée à un score de priorité commun 0-100 :
 *
 * | Source                        | Score unifié                              |
 * |-------------------------------|-------------------------------------------|
...
/**
 * Construit la liste unifiée pour un utilisateur.
 * Retourne { items (triés, dédupliqués, non tronqués), counts, pendingEmailIds }.
 */
async function buildTodayList(userId) {
  const now = Date.now();
  const items = [];
